...
async function testScorecardV6Proper() {
  const testData = {
    UserInformation: {
      Industry: "Technology",
      UserName: "John Doe",
...
2. **"Advanced AI Tools" (Workshop)**
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('✅ Scorecard V6 endpoint working with proper data!');
      console.log(`📄 Generated HTML length: ${html.length} characters`);
      
      // Save to file
      const fs = require('fs');
      fs.writeFileSync('test-scorecard-v6-proper-output.html', html);
      console.log('💾 HTML saved to test-scorecard-v6-proper-output.html');
      
      // Check if it contains expected content
      if (html.includes('AI Efficiency Scorecard') && html.includes('Enabler')) {
        console.log('✅ HTML contains expected scorecard content!');
      } else {
        console.log('⚠️  HTML may not contain expected content');
      }
    } else {
